...
module.exports.imdb = async function(name) {
	if(!name) throw new Error("[Popcat Wrapper] imdb(name) ==> 'name' paramter is missing")
...
module.exports.simpstamp = async function(image) {
	if(!image) throw new Error("[Popcat Wrapper] The field 'image' was left empty in the SIMPTAMP function!")
 const input = `image=${encodeURIComponent(image)}`
 const res = await request('simpstamp', input)
 return res;
}
